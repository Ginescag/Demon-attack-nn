...
// ///////////////////////////////////////////////////////////////////////////////
// /// RL AGENT FUNCTIONS (Usadas en modo train/eval)
// ///////////////////////////////////////////////////////////////////////////////
...
//     double threat_x_pos = 0.0;
//     for (int i = 0; i < 8; ++i) {
//         int bullet_y_addr = 0x50 + i;
//         int bullet_x_addr = 0x58 + i;
//         int bullet_y = ram.get(bullet_y_addr);
//         if (bullet_y > 100 && bullet_y < 200) { 
//             imminent_threat = 1.0;
//             threat_x_pos = static_cast<double>(ram.get(bullet_x_addr));
//             break;
//         }
//     }
//     features.push_back(imminent_threat);
//     if (imminent_threat > 0.5) {
//         features.push_back((threat_x_pos - player_x) / 160.0);
//     } else {
//         features.push_back(0.0);
//     }
//     features.push_back((ram.get(28) == 0x01) ? 1.0f : 0.0f);
//     features.push_back(static_cast<double>(ram.get(114)) / 5.0f);
...
//    std::printf("  eval: evaluar el modelo entrenado\n");
//    std::printf("  manual: controlar el juego manualmente y monitorizar la RAM\n");
// }
...
//    // --- CONFIGURAR ALE ANTES DE CARGAR LA ROM ---
...
//    } else {
//       // Para eval, manual, o entrenamiento con visualización
//       alei.setBool("display_screen", true);
//       if (manual_mode) {
//          alei.setFloat("repeat_action_probability", 0.0);
//       }
//    }

//    // --- CARGAR LA ROM DESPUÉS DE LA CONFIGURACIÓN ---
//    alei.loadROM(argv[1]);

//    // --- MODO DE CONTROL MANUAL ---
//    if (manual_mode) {
//       clearScreen();
//       std::cout << "--- MODO DE CONTROL MANUAL ---" << std::endl;
//       std::cout << "Controles: Flechas para mover, Espacio para disparar, ESC para salir." << std::endl;
//       SDL_Delay(2000);

//       while (true) {
//          printRAM(alei);
//          std::cout << "\nVidas: " << alei.lives() << " | Puntuación: " << alei.getEpisodeFrameNumber() << std::endl;

//          SDL_Event event;
//          while (SDL_PollEvent(&event)) {
//             if (event.type == SDL_QUIT || (event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_ESCAPE)) {
//                std::cout << "Saliendo del modo manual." << std::endl;
//                return 0;
//             }
//          }
...
//          if (left && fire) action = PLAYER_A_LEFTFIRE;
//          else if (right && fire) action = PLAYER_A_RIGHTFIRE;
//          else if (left) action = PLAYER_A_LEFT;
//          else if (right) action = PLAYER_A_RIGHT;
//          else if (fire) action = PLAYER_A_FIRE;

//          alei.act(action);

//          if (alei.game_over()) {
//             printRAM(alei);
//             std::cout << "\n¡JUEGO TERMINADO! Reiniciando en 2 segundos..." << std::endl;
//             SDL_Delay(2000);
//             alei.reset_game();
//          }
...
//    const int NUM_EPISODES = evaluation_mode ? 100 : 10000;
//    const size_t REPLAY_MEMORY_SIZE = 10000;
//    const size_t BATCH_SIZE = 64;
//    const double GAMMA = 0.99;
//    double epsilon = evaluation_mode ? 0.0 : 0.0;
//    const double EPSILON_MIN = 0.1;
//    const double EPSILON_DECAY = 0.9997;
//    const int TRAIN_FREQUENCY = 4;
//    const double FORCE_MOVE_PROB = 0.15;
...
//    if (training_mode) std::cout << "Iniciando entrenamiento rápido..." << std::endl;
//    else if (evaluation_mode) std::cout << "Iniciando modo de EVALUACIÓN..." << std::endl;
//    else std::cout << "Iniciando entrenamiento con visualización..." << std::endl;
...
//          int action_idx;
//          if (!evaluation_mode) {
//             if (static_cast<double>(gen()) / gen.max() < epsilon) {
//                 action_idx = gen() % NUM_ACTIONS;
//             } else if (static_cast<double>(gen()) / gen.max() < FORCE_MOVE_PROB) {
//                 auto q_values = model.predict(state);
//                 action_idx = (q_values[0] > q_values[1]) ? 0 : 1;
//             } else {
//                 auto q_values = model.predict(state);
...
//              lives = alei.lives();
//          }
//          double imminent_threat = next_state[3];
//          double threat_relative_pos = next_state[4];
//          if (imminent_threat > 0.5) {
//              if (threat_relative_pos < 0 && (action_idx == 1 || action_idx == 4)) shaped_reward += 10.0;
//              else if (threat_relative_pos > 0 && (action_idx == 0 || action_idx == 3)) shaped_reward += 10.0;
//          }
//          if (ram.get(126) == 0x4E) shaped_reward += 50.0;
...
//             if (replay_memory.size() > REPLAY_MEMORY_SIZE) replay_memory.pop_front();
...
//       } else {
//          std::cout << "Episodio: " << episode << ", Puntuacion: " << total_score << std::endl;
//       }
